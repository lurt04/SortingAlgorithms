...
/* maak het array */
void createArray(unsigned short int *array, unsigned short int arrayLength, unsigned short int& selectedList){
	...
	if (selectedList == 1) { // zet het random bestand in het array.
		std::string textFile = "C:\\Programming\\algoritmes\\InputFiles\\randomSorted.txt";
		...
		std::string textFile = "C:\\Programming\\algoritmes\\InputFiles\\reverseSorted.txt";
		File.open(textFile.c_str());
		std::cout << "Reverse sorted selected. \n";
	}

	else if(selectedList == 3) { // zet het "nearly sorted" bestand in het array.
		std::string textFile = "C:\\Programming\\algoritmes\\InputFiles\\nearlySorted.txt";
		File.open(textFile.c_str());
...
void outputToFile(unsigned short int *array, unsigned short int arrayLength, unsigned short int selectedList){
	std::ofstream File;
	std::string fileFinishedRandom = "C:\\Programming\\algoritmes\\OutputFilesMergeSort\\fileFinishedRandom.txt";
	std::string fileFinishedReverse = "C:\\Programming\\algoritmes\\OutputFilesMergeSort\\fileFinishedReverse.txt";
	std::string fileFinishedNearly = "C:\\Programming\\algoritmes\\OutputFilesMergeSort\\fileFinishedNearly.txt";

	if (selectedList == 1){
		File.open(fileFinishedRandom.c_str()); 
	}
	else if (selectedList == 2){
		File.open(fileFinishedReverse.c_str()); 
	}
	else if (selectedList == 3){
		File.open(fileFinishedNearly.c_str()); 
	}

	for (int i = 0; i <= arrayLength; i++) {
        File << array[i] << " "; // zet het array in het bestand.
    }
	std::cout << "\n";
    File.close();
}
...
int main(){
	...
	createArray(array, arrayLength, selectedList);
	auto startTime = std::chrono::high_resolution_clock::now();
	mergeSort(array, begin, end);
	printArray(array, arrayLength);
	outputToFile(array, arrayLength, selectedList);
